...
int main(int argc, char *argv[]) {
	...
	ColorSensor *value_color = malloc(sizeof(ColorSensor) * 1);
	Acceleration *value_acc = malloc(sizeof(Acceleration) * 1);
	Sample value_sample;
	...
	while (end_of_program != 1) {
		...
		sendto(sockfd, &value_sample, sizeof(Sample), 0, &serv_addr, sizeof(serv_addr));
